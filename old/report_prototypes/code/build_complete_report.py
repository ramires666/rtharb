"""Complete standalone report generator with 100% self-contained data.

Embeds real 1-minute Alpaca candlestick data, interactive timeframe switcher (1m, 5m, 15m),
and updated Production Strategy (Time-Stop 120m + Stop-Loss 1.5% + 4σ Lockout).
"""
...
def build_and_write():
    print("⏳ Loading Alpaca 1m historical bars...")
    ...
    # Pick 10 high-interest sessions (profitable, stop loss, time stop, normal)
    sample_days = [
        "2024-08-21", "2024-08-22", "2024-08-27", "2024-09-03", "2024-09-11",
        "2024-09-24", "2024-10-04", "2024-11-06", "2025-01-08", "2025-03-14"
    ]
    ...
    for d_str in sample_days:
        ...
        trs = [tr for tr in trades_prod if tr["entry_time"].startswith(d_str)]
        pnl_val = sum(float(tr["pnl_str"].replace("$", "").replace("+", "").replace(",", "")) for tr in trs)
        sign = "+" if pnl_val >= 0 else ""
        desc = f"Сделок: {len(trs)} | PnL: {sign}${pnl_val:,.2f}"
    ...
    # Generate full self-contained HTML
...
<body>
    <div class="container">
        ...
        <div class="header">
            <div>
                ...
                <p>100% Реальные данные Alpaca (2024–2026) | Оптимизированная стратегия: Time-Stop 120m + Stop-Loss 1.5% + 4σ Lockout</p>
            </div>
            <div>
        ...
        <div class="section-card">
            <div class="section-title">
                <span>📈 1. Сравнительный график Equity (Продакшн с фильтрами vs Базовые варианты)</span>
            </div>
                ...
                <table>
                    ...
                    <tbody>
                        <tr class="highlight-row">
                            ...
                            <td>1,320</td>
                            <td><strong>-$348.60</strong> (хвост срезан!)</td>
                        </tr>
                        <tr>
        ...
        <div class="section-card">
            <div class="section-title">
                ...
                <span style="font-size: 14px; font-weight: 500; color: var(--accent-cyan);">⚡ 100% Автономно и без сбоев</span>
            </div>
            <div class="section-subtitle">
                Выберите любую дату. График строится мгновенно. Переключайте таймфрейм (1m / 5m / 15m) для проверки микроструктуры:
            </div>
    ...
    <script>
        const appData = {json.dumps(embedded_payload)};
        ...
        function initDaySelector() {{
            ...
            const dayKeys = Object.keys(appData.sessions);
        ...
        // 4. ON-THE-FLY RESAMPLING ENGINE
        ...
        // 5. RENDER INTRADAY PRICE & Z-SCORE CHART
