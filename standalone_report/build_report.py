"""Self-contained generator for the Standalone Stat-Arb HTML One-Pager."""
...
def build_report():
    print("1. Loading 2-year 1m data...")
    ...
    # 6. Build Metrics Comparison Table Rows
    metrics_rows = ""
    ...
    # 8. Build Daily Sessions (top recent sessions and active trade sessions)
    all_trade_dates = sorted(trades_df["entry_time"].dt.date.unique()) if not trades_df.empty else []
    # Take last 60 active trade sessions to keep HTML under 2MB and ultra fast
...
"""
    ...
    print(f"🎉 SUCCESS! Written index.html: {out_file.stat().st_size:,} bytes")
