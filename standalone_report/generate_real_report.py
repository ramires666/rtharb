"""Generates index.html with 100% REAL Alpaca historical candles and backtest trades."""
...
def generate_real_report():
    print("⏳ Loading real 2-year Alpaca 1m cache...")
    ...
    print("⏳ Calculating real fair value & Z-scores...")
    ...
    # 1. Real Equity Curves (sampled every 10 bars = 10 mins)
    ...
    # 4. Real Daily Sessions (All trading sessions with real Alpaca 1m bars)
    # We include all sessions that had trades in Scenario B
    days_with_trades = sorted(trades_b["entry_time"].dt.date.unique())
    print(f"⏳ Formatting real Alpaca candlestick data for {len(days_with_trades)} trade sessions...")
    ...
    for d in days_with_trades:
        ...
        # Extract REAL Alpaca 1m OHLC and Fair Value
    ...
    # Save real dataset to a JSON file so index.html loads real data instantly
    json_path = project_root / "standalone_report" / "real_data.json"
    full_data = {
        "equity": equity_payload,
        "metrics_rows": metrics_rows,
        "trades": trades_payload,
        "daily": daily_payload,
        "days": [str(d) for d in days_with_trades]
    }
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(full_data, f)
    print(f"✅ Real data exported to JSON: {json_path.stat().st_size:,} bytes")

    # Generate complete self-contained index.html with real data embedded directly
    html_path = project_root / "standalone_report" / "index.html"
    generate_html(full_data, html_path)


def generate_html(data, out_path):
    equity_json_str = json.dumps(data["equity"])
    daily_json_str = json.dumps(data["daily"])
    trades_json_str = json.dumps(data["trades"])
    days_json_str = json.dumps(data["days"])
...
<head>
    ...
    <title>Внутридневной Стат-Арбитраж: NVDA vs QQQ (Реальные данные Alpaca 2 Года)</title>
    <!-- Plotly JS CDN for High Resolution Interactive Charts -->
...
<body>
    <div class="container">
        ...
        <div class="header">
            <div>
                ...
                <p>Реальные исторические 1-минутные данные Alpaca (2024–2026) | 195,502 баров | 502 торговых сессии</p>
            </div>
            <div>
        ...
        <div class="section-card">
            <div class="section-title">📈 1. Общий сравнительный график Equity (Все 4 Стратегии на одном полотне)</div>
            ...
            <div style="margin-top: 24px;">
                <div class="section-title" style="font-size: 18px;">📋 Сводная таблица эффективности 4-х режимов (Реальный бэктест)</div>
                <table>
                    <thead>
                        <tr>
                            <th>Сценарий</th>
        ...
        <div class="section-card">
            <div class="section-title">🔍 2. Крупный инспектор внутридневных сделок и свечей (Реальные котировки Alpaca)</div>
            <div class="section-subtitle">
                Выберите любую реальную торговую сессию за 2 года. График строится по реальным 1-минутным свечам Alpaca, желтой линии справедливой цены ($QQQ \times \beta$), точкам входа после подтвержденного разворота $\delta$ и выходам:
            </div>
        ...
        <div class="section-card">
            <div class="section-title">📜 3. Полный реестр совершенных сделок (Стратегия B: Входы, Выходы, Прибыли и Убытки)</div>
            <div class="section-subtitle">
                Все 1,320 реальных сделок (включая прибыльные и убыточные, закрытия по тейк-профиту и принудительные закрытия в 15:55 ET):
            </div>
    ...
    <script>
        ...
        const dailySessions = {daily_json_str};
        const tradesData = {trades_json_str};
        const daysList = {days_json_str};
        ...
        function initDaySelector() {{
            ...
            if (daysList.length > 0) {{
                // Default to first active session with interesting trades
...
"""
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"🎉 Standalone HTML One-Pager generated: {out_path.stat().st_size:,} bytes")
